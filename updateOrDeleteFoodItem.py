...
def delete_food_item(food_items, index):
    "deletes a registered food item "
    food_item_name = food_items[index]['name']
    confirmation = input(f"Are you sure you want to permanently delete {food_item_name}? (y/n): ").strip().lower()
    ...
    if confirmation == 'y':
        ...
        print(f"Food item {food_item_name} deleted successfully.")
    else:
        print(f"Deletion of {food_item_name} canceled.")

def main():
    food_items = display_food_items()
    choice = input("Enter the number of the food item to update/delete (or 0 to go back): ")

    # display choices and prompt user for choice.
